...
function substitute(str) {
	const subs = [
		...
		// { from: 'qalculate-gtk', to: 'calculator'},
		...
		widget.set = (clientJson, screenCoords) => {
			...
			if (c) {
				if (c.attribute?.ws !== clientJson.workspace.id) {
					...
					// TODO try and upstream this
					// Recaluate scaled size if needed
					if (c.attribute.x != clientJson.at[0]) {
						let new_x = clientJson.at[0];
						let w = clientJson.at[0];
						if (screenCoords.x != 0) new_x -= screenCoords.x;

						if (new_x + w <= 0) new_x += (Math.floor(new_x / SCREEN_WIDTH) * SCREEN_WIDTH);
						else if (new_x < 0) { w = new_x + w; new_x = 0; }

						if (new_x + w > SCREEN_WIDTH) w = SCREEN_WIDTH - new_x;
						c.attribute.x = new_x;
					}

					if (c.attribute.y != clientJson.at[1]) {
						let new_y = clientJson.at[1];
						let h = clientJson.at[1];
						if (screenCoords.y != 0) new_y -= screenCoords.y;

						if (new_y + self.attribute.h <= 0) new_y += (Math.floor(new_y / SCREEN_HEIGHT) * SCREEN_HEIGHT);
						else if (new_y < 0) { h = new_y + h; new_y = 0; }

						if (new_y + h > SCREEN_HEIGHT) h = SCREEN_HEIGHT - new_y;
					...
					fixed.attribute.move(c,
						...
						Math.max(0, c.attribute.x * OVERVIEW_SCALE),
						Math.max(0, c.attribute.y * OVERVIEW_SCALE)
					);
					return;
				}
			}
